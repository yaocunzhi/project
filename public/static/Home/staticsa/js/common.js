	                ...
	                ajax: {
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		if(data.ww != ''){
		                			_info += '<a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&amp;uid='+data.ww+'&site=cntaobao&s=2&charset='+_CHARSET+'" ><img border="0" src="http://amos.alicdn.com/online.aw?v=2&uid='+data.ww+'&site=cntaobao&s=2&charset='+_CHARSET+'" alt="点击这里给我发消息" style=" vertical-align: middle;"/></a>';
		                		}
		                		if(_info == ''){
		                			_info = '--';
		                		}
...
function addcart(goods_id,quantity,callbackfunc) {
    ...
    var need_service = $('dl[nctype="nc-service"]').find('a').hasClass('hovered') ? 1 : 0;//获取安装服务选项 - Dino
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity ,'need_service':need_service}, function(data) {//提交安装服务选项 - Dino
        if (data != null) {
            if (data.state) {
            	if(callbackfunc){
