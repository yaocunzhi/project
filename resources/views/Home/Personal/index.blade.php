...
<link href="/static/Home/static/css/user_style.css" rel="stylesheet" type="text/css" />
...
<script src="/static/Home/static/js/iCheck.js" type="text/javascript"></script>
<script src="/static/Home/static/js/custom.js" type="text/javascript"></script>
<title>订单管理</title>
</head>
 ...
 <div class="user_center clearfix">
   <div class="left_style">
     ...
     <div class="user_portrait">
      <a href="#" title="修改头像" class="btn_link"></a> <img src="/static/Home/static/images/people.png">
      <div class="background_img"></div>
      </div>
      <div class="user_name">
       <p><span class="name">化海天堂</span></p>
 ...
 <!--右侧样式-->
  ...
  <div class="title_style"><em></em>订单管理</div> 
   <div class="Order_form_style">
      <div class="Order_form_filter" id="title">
       <a href="javascript:void(0)" class="current">全部订单（23）</a>
       <a href="javascript:void(0)" class="">未付款（2）</a>
       <a href="javascript:void(0)" class="">待发货（3）</a>
       <a href="javascript:void(0)" class="">待收货（5）</a>
       <a href="javascript:void(0)" class="">退货/退款（0）</a>
       <a href="javascript:void(0)" class="">交易成功（0）</a>
       <a href="javascript:void(0)" class="">交易关闭（0）</a>
      </div>
     ...
     <div class="right_search"><input name="" type="text"  class="add_Ordertext" placeholder="请输入产品标题或订单号进行搜索"/><input name="" type="submit" value="搜索订单"  class="search_order"/></div>
      ...
      <div class="Order_form_list">
         ...
         @foreach($order as $row)
          @foreach($pp as $s)
          @foreach($s as $ss)
          @if($ss->order_id == $row->id)
         <tbody>       
           <tr class="Order_info"><td colspan="6" class="Order_form_time"><input name="" type="checkbox" value=""  class="checkbox"/>订单号:{{$row->order_num}}<em></em></td></tr>
           ...
           <td>
            ...
            <a href="#" class="product_img"><img src="{{$ss->pic}}" width="80px" height="80px"></a>
            <a href="3" class="p_name"></a>
            <p class="specification">{{$ss->oname}}</p>
            </div>
            </td>
            <td>5</td>
           <td>{{$ss->onum}}</td>
           ...
           <td class="split_line"><p style="color:#F30">  
            @if($row->status==0) 未付款 
            @elseif($row->status==1) 待发货 
            @elseif($row->status==2) 已付款 
            @elseif($row->status==3) 已发货
            @elseif($row->status==4) 已收货
            @elseif($row->status==5) 待评价
            @endif
            </p></td>
          ...
          @if($row->status==4) 待评价
           <td class="operating">
             <a href="/pinlun/create/{{$ss->gid}}">评价</a>
             <a href="#">联系客服</a>
             <a href="#">删除</a>
